  ...
  return (
    <section className="page">
      ...
      <div className="metrics-grid">
        {!telemetryUnavailable ? (
          <>
            ...
            <MetricCard label={t("Cost (Credits)")} value={integer(Math.round((data?.total_microcredits ?? 0) / MICRO_PER_CREDIT))} detail={t("Recorded consumption")} />
      ...
      <section className="panel">
        <div className="panel-heading">
          <h2>{t("Quota watch")}</h2>
          <span>{t("Keys above 80% of request, token or Credits quota")}</span>
        </div>
            ...
            <tbody>
              {(data?.risk_keys ?? []).map((key) => (
                <tr key={key.id}>
                  ...
                  <td>{percent(key.cost_used_ratio ?? 0)}</td>
